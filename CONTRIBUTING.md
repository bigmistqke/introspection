...
See `CLAUDE.md` for TypeScript and testing conventions. Key rules:

- **No `as never` casts.** Use a proper type, a narrower cast (`as Record<string, unknown>`, `as object`), or a typed wrapper.
- **No abbreviated variable names.** Write `parameters` not `params`, `error` not `err`, `event` not `evt`, `result` not `res`.
- **Test actual behaviors, not type shapes.** Anything the type checker catches doesn't need a test.
- **No mocking.** Tests run against real implementations (real browsers, real file I/O).
