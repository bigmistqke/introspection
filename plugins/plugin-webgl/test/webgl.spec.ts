...
async function makeTrace(page: Page) {
...
test('uniform1f push event has correct name, value, and glType', async ({ page }) => {
  const { outDir, plugin, handle } = await makeTrace(page)
  await setupGL(page)
...
test('captureCanvas({ contextId }) captures only the matching canvas', async ({ page }) => {
  ...
  const [traceId] = await readdir(outDir)
  const eventsPath = join(outDir, traceId, 'events.ndjson')
