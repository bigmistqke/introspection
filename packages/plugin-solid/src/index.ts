...
async function captureState(context: PluginContext): Promise<void> {
  ...
  const state = (await context.page.evaluate(() => {
    return (
      (
        window.__introspect_plugins__ as
          | { solid?: { getState?(): unknown } }
          | undefined
      )?.solid?.getState?.() ?? null
    );
  })) as SolidState | null;
...
export function solidDevtools(
  options?: SolidDevtoolsOptions,
): IntrospectionPlugin {
  return {
    name: "solid",
    ...
    async install(context: PluginContext): Promise<void> {
      ...
      await context.page.evaluate((pluginOptions) => {
        (
          window.__introspect_plugins__ as
            | { solid?: { configure?(config: unknown): void } }
            | undefined
