...
function summariseBody(raw: string): BodySummary {
  ...
  const keys = Object.keys(parsed).filter(k => ERROR_KEYS.has(k))
  ...
  for (const [k, v] of Object.entries(parsed)) {
    if (Array.isArray(v)) {
      const first = v[0] && typeof v[0] === 'object' ? Object.keys(v[0] as object).filter(ik => ERROR_KEYS.has(ik)) : []
      arrays[k] = { length: v.length, itemKeys: first }
    } else if (ERROR_KEYS.has(k)) {
