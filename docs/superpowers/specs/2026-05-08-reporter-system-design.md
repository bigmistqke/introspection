...
A consumer-side counterpart to plugins. **Plugins capture; reporters consume.** Reporters subscribe to the session's live event stream during a run and produce derived artifacts: summary files, JUnit XML, downstream tooling input, custom dashboards.
...
Introspection captures a rich event stream per session, but today the *only* output is `events.ndjson` + `assets/` + `meta.json`. Anything that wants a different shape (per-test summary, run index, JUnit XML, custom HTML viewer) has to:
...
The bus already exists (`SessionBus` in `packages/types/src/index.ts:716`) and `createSessionWriter` already exposes it (`packages/write/src/session.ts:119`). Plugins can subscribe via `PluginContext.bus`. What's missing is a **first-class reporter abstraction** with per-test slicing and lifecycle hooks so consumers don't reinvent the test-boundary bookkeeping.

## Scope

**In scope:**

- `Reporter` (type `IntrospectionReporter`) interface — registered alongside plugins in `createSessionWriter` and via `withIntrospect`.
- Per-event callback (`onEvent`) and lifecycle callbacks (`onSessionStart`, `onTestStart`, `onTestEnd`, `onSessionEnd`).
...
- `finalize()` waits for all reporters' async callbacks to drain — disk writes complete before the session is considered finalized.
...
- **Cross-session reporters** (aggregating multiple sessions). Users can run any post-hoc tooling against multiple run directories themselves.
...
`withIntrospect` forwards `reporters` to each per-test `createSessionWriter` call in the worker. There is no runner-side reporter; reporters are instantiated once per worker process and receive per-test lifecycle callbacks for every test that worker runs.
...
export interface ReporterContext {
  sessionId: string
  outDir: string                  // session directory (.introspect/<run-id>/<test-id>)
...
- Plugins **install** browser-side scripts, attach CDP listeners, mutate the page or session.
...
- **Session ends mid-test** (interrupt, crash) — the in-flight test's slice is delivered to `onTestEnd` with `status: 'interrupted'` synthesized.
...
- `createSessionWriter(options)` — accept `reporters?: IntrospectionReporter[]`. Pass through to internal lifecycle handler.
...
A reporter throwing must not crash the session or the test run. Same policy as plugins (see `2026-04-13-robust-failure-handling-design.md`):

- Errors from `onEvent` / `onTestEnd` / etc. are caught, surfaced as `introspect:warning` events on the bus (`source: 'reporter'`, with the reporter name), and the reporter is **disabled for the rest of the session**. Other reporters keep running.
...
- **Backpressure.** If a reporter is slow, the in-memory event queue grows. For now, document the constraint ("reporters should be cheap; defer heavy work to `onSessionEnd`"). Revisit if it bites.
