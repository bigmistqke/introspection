...
- **Plugin-contributed CLI commands** — plugins register their own `introspect <plugin>` subcommands (e.g. `introspect webgl`, `introspect webgl uniforms --name u_time`). The CLI discovers installed plugins and loads their commands at startup. This lets Claude interact with plugin data directly in the terminal without writing code — `introspect webgl --session <id>` instead of `introspect eval 'events.filter(...)'`. Two discovery approaches: (A) scan `node_modules/@introspection/plugin-*` at startup; (B) explicit config file written by `introspect plugin add`. The plugin interface would gain an optional `commands?: PluginCommand[]` field alongside the existing `name`, `script`, `install`, `capture`.
